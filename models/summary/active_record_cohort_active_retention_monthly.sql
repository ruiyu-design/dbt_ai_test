...
user_first_active as (
    select
        ...
        min_by(goods_plan_type, record_month) as first_month_plan_type,
        min_by(signup_country, record_month) as country_group
    from dbt_models_details.stg_active_user_records_daily a
    left join month_plans b on a.workspace_id=b.workspace_id and a.record_month=b.month
    group by 1
),
paid_user_first_active as (
    select
        a.uid,
        min(record_month) as first_paid_active_month,
        min_by(workspace_id, record_month) as workspace_id,
        min_by(signup_country, record_month) as country_group
    from dbt_models_details.stg_active_user_records_daily a
...
cohort_users as (
    ...
    where first_active_month >= '2024-01-01'
        and first_month_plan_type is null
    ...
    select
        ...
        a.first_paid_active_month as cohort_month,
        a.country_group,
        concat(
        case
    ...
    join month_plans b on a.workspace_id = b.workspace_id and a.first_paid_active_month = b.month
    where
        fa.first_active_month >= '2024-01-01'
        and a.first_paid_active_month >= fa.first_active_month
),
...
all_activity as (
    ...
    where record_month < date_trunc(current_date(), MONTH)
    group by 1, 2, 3
union all
    select
        uid,
...
-- =================================================
-- 7. [新增] 确定用户 Month 0 的属性 (画像定格)
--    我们需要先知道每个用户在初始月是用什么设备、是不是Meeting User
-- =================================================
...
skeleton as (
    ...
    -- 过滤掉未来的时间
    where date_add(i.cohort_month, interval ms.month_diff month) <= date_trunc(current_date(), month)
),
...
final_dataset as (
    select
        ...
        s.plan_type,
        s.device,
        s.is_meeting_active_user, -- 这里的含义是：该用户在 Month 0 时是 Meeting User
        s.month_diff,
...
select
    *,
    ...
    lead(retained_active_users, 1, 0) over (
        partition by cohort_type, cohort_month, country_group, plan_type, device, is_meeting_active_user
...
from final_dataset
order by cohort_type, cohort_month, device, is_meeting_active_user, month_diff