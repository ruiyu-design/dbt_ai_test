...
-- 1. 满足：免费用户,付费前音频均不需要说话人,付费前音频均小于7200s，付费前音频均为英文文件转写 进入测试
-- 2. ws_id>=50, 英伟达;
-- 2. ws_id<50, small.en;
...
and signup_date>='2025-07-12'
and first_transcription_type='File'-- 首个转写是文件转写
and first_transcription_language='en-US'-- 首个转写语言是英语
),
...
record as (
	...
	select
		...
		,device_category
		,source
		,count(case when transcribe_language!='en-US' or transcription_type!=1 or enable_speaker=1 or audio_duration>=7200 then 1 else null end) as match_other_records
		,count(case when transcribe_language='en-US' and transcription_type=1 and enable_speaker=0 and audio_duration<7200 and right(cast(workspace_id as string),2)>='50' then 1 else null end) as nvidia_records
		,count(case when transcribe_language='en-US' and transcription_type=1 and enable_speaker=0 and audio_duration<7200 and right(cast(workspace_id as string),2)<'50' then 1 else null end) as smallen_records
...
select
  ...
  ,case when match_other_records>0 then 'mixed records'
	 when nvidia_records>0 then 'nvidia only'
	 when smallen_records>0 then 'smallen only'
	 else 'else'
	 end as is_canary
